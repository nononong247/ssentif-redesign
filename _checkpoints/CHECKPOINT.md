...
> **마지막 갱신**: 2026-07-30 (요금제 4카드 재설계 배포 완료 + **결제 방식 정책 확정**. 미커밋: FAQ 결제 문구 수정(pricing·consulting)·`BILLING_POLICY_HANDOFF.md`·CLAUDE.md 결제 규칙·이 문서·brand-kit.html)
...
- FREE 차이(2026-07-30 개정): 무제한 회원 프로필 등록 / **회원 연동 월 2회**(매월 1일 리셋·성공 시 차감 — 구 "3명까지" 폐기) / 데이터 조회 최근 30일 / 제한된 AI 기능 사용.
...
### 결제 방식 정책 확정 (2026-07-30 후속 · 미배포)

케이스별 결제 플로우 6건을 사용자가 확정 → **`_checkpoints/BILLING_POLICY_HANDOFF.md`**(개발자 전달용, 케이스 A~K + 금액 예시)로 정리. CLAUDE.md §요금제에도 요약 반영.

핵심: ①추가 인원·차액 **즉시 일할 결제** 후 다음 결제일 합산 정상가 ②감소는 환불 없음 ③좌석 승계 없음 ④TEAM 1인 잔류 시 자동 STANDARD ⑤강등 수동만 ⑥결제 실패 3일 재시도(오전 11시) 후 무료 강등 ⑦앵커일 영구 축소(31→30→28).
...
- **정책 정본이 HTML 문서로 이동**: `~/Desktop/센티프 콘텐츠/02_기획/센티프-결제정책.html` (라이트 브랜드 디자인, 사용자 검토·다듬기용. 현재 **v4**, 미결 0건). 확정 선언 나오면 `BILLING_POLICY_HANDOFF.md` 재작성 + FAQ 최종 대조.
...
- FREE 제한 3종(연동 기준 2026-07-30 개정): 회원 연동 **월 2회**(매월 1일 리셋) / 조회 최근 30일 / AI 제한. **회원 프로필 등록·연동된 회원 수는 무제한**.
...
- `BILLING_POLICY_HANDOFF.md` — 🔴 **구버전(v1 시점)**. 정본은 `~/Desktop/센티프 콘텐츠/02_기획/센티프-결제정책.html`(v4, 검토 중) — 확정 후 이 md를 재작성 예정
